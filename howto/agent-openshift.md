...
4. Delete the secrets by deleting the service account.
   ...
   {: pre}
    ...
    `-t`
    :   Cluster type. Use this option to define the cluster type where the {{site.data.keyword.logs_routing_full_notm}} agent is deployed. Choose `OpenShift` for removing an agent from an {{site.data.keyword.openshiftlong_notm}} cluster.
